/* =========================================================
   proveedores.js — Módulo de Proveedores para el Panel BUNKER
...
(function () {
  ...
  // ── Firestore helpers for servicios subcollection ──
  function _serviciosRef(proveedorId) {
    return BNK_FIREBASE.db.collection('proveedores').doc(proveedorId).collection('servicios');
  }
  ...
  function abrirModal(proveedorData, modo) {
    ...
    // Mostrar loading mientras se buscan cotizaciones
    ...
    cotHtml += '<div id="prvCotizaciones"><span style="color:var(--tx-muted);font-size:12px">Cargando...</span></div>';
    wrap.innerHTML = cotHtml;

    if (prvNombreComercial || prvRazonSocial || prvId) {
  ...
  function guardarProveedor() {
    // Permission check
    if (!BNK_AUTH.canEdit('proveedores')) {
      BNKToast.warn('No tienes permisos para editar proveedores.');
      return;
    }
    ...
    if (esNuevo) {
      ...
      promise = BNK_DB.proveedores.create(data);
    } else {
      promise = BNK_DB.proveedores.update(id, data);
    }
  ...
  function eliminarProveedor(id) {
    // Permission check
    if (!BNK_AUTH.canEdit('proveedores')) {
      BNKToast.warn('No tienes permisos para eliminar proveedores.');
      return;
    }
  ...
  function load() {
    ...
    return BNK_DB.proveedores.list()
      .then(function (data) {
        if (prvLoading) prvLoading.style.display = 'none';
        _proveedores = data || [];
        _cargado = true;
  ...
  // ══════════════════════════════════════════════════════════
  // TAB 5 — SERVICIOS (subcollection: proveedores/{id}/servicios)
  // ══════════════════════════════════════════════════════════
  ...
  // ── Guardar nuevo servicio (Firestore subcollection) ──
  function _guardarNuevoServicio(tr) {
    if (!BNK_AUTH.canEdit('proveedores')) {
      BNKToast.warn('No tienes permisos para agregar servicios.');
      return;
    }
    ...
    _serviciosRef(proveedorId).add(data)
      .then(function () {
        loadServicios(proveedorId);
      })
      .catch(function (err) {
        if (saveBtn) { saveBtn.disabled = false; saveBtn.innerHTML = '&#10003;'; }
        BNKToast.error('Error al guardar servicio: ' + err.message);
      });
  }
  ...
  // ── Guardar edición de servicio existente (Firestore subcollection) ──
  function _guardarEdicionServicio(tr, srvId) {
    if (!BNK_AUTH.canEdit('proveedores')) {
      BNKToast.warn('No tienes permisos para editar servicios.');
      return;
    }
    ...
    _serviciosRef(proveedorId).doc(srvId).update(data)
      .then(function () {
        loadServicios(proveedorId);
      })
      .catch(function (err) {
        if (saveBtn) { saveBtn.disabled = false; saveBtn.innerHTML = '&#10003;'; }
        BNKToast.error('Error al actualizar servicio: ' + err.message);
      });
  }

  // ── eliminarServicio (Firestore subcollection) ──
  function eliminarServicio(srvId) {
    if (!BNK_AUTH.canEdit('proveedores')) {
      BNKToast.warn('No tienes permisos para eliminar servicios.');
      return;
    }
    ...
    BNKConfirm.show('\xBFEliminar este servicio?\nEsta acci\xF3n no se puede deshacer.')
      .then(function (confirmado) {
        ...
        _serviciosRef(proveedorId).doc(srvId).delete()
          .then(function () {
